...
import { X, Mail, Lock, UserPlus, Eye, EyeOff, User } from 'lucide-react';
...
export default function RegisterPage({ onClose, switchToLogin }: RegisterPageProps) {
      ...
      <div className="relative w-full max-w-md mx-4 bg-triton-card border border-triton-border rounded-2xl p-8 shadow-2xl">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 bg-triton-gold/10 rounded-full mb-4">
            <UserPlus className="w-7 h-7 text-triton-gold" />
          </div>
          <h2 className="text-2xl font-bold text-triton-text">{t('auth.register')}</h2>
          <p className="text-triton-text-muted mt-1">{t('auth.registerDesc')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-triton-text-muted mb-1.5">{t('auth.nickname')}</label>
            <div className="relative">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-triton-text-muted" />
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder={t('auth.yourNickname')}
          ...
          <div>
            <label className="block text-sm font-medium text-triton-text-muted mb-1.5">{t('auth.confirmPassword')}</label>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-triton-text-muted" />
              <input
                type="password"
        ...
        <div className="mt-6 text-center">
          <span className="text-triton-text-muted text-sm">{t('auth.hasAccount')}</span>
          <button
